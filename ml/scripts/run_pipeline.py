...
from ml.config.settings import CLEANED_DATASET_CSV, FEATURE_COLUMNS, RAW_DATASET_CSV, TARGET_COLUMN
...
def run(input_path: str, version: str) -> None:
    ...
    y = engineered["price_usd"] if "price_usd" in engineered else engineered[TARGET_COLUMN]
    ...
    winner, _ = ModelTrainer().train_and_select(x, y, build_preprocessing_pipeline(numeric, categorical))
    bundle = {"model": winner.model, "feature_engineer": engineer, "feature_columns": columns}
    save_model(bundle, version)
    track_training(winner.model, winner.metrics, {"model_name": winner.name, "version": version})
    save_metadata({"version": version, "model_name": winner.name, "feature_columns": columns, "metrics": winner.metrics, "trained_at": datetime.now(timezone.utc).isoformat()})
