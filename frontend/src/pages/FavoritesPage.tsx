...
export function FavoritesPage({ onNotify }: FavoritesPageProps) {
  const { favoritesQuery, removeFavorite, filter, setFilter, sortBy, setSortBy, filteredFavorites } = useFavorites(onNotify);
    ...
    <section className="page-section">
      ...
      <div className="dashboard-grid">
        <label className="field search-filter">
          ...
          <input value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="Search by airline or route" />
        </label>
        <label className="field search-filter">
          <span className="field-label">Sort by</span>
          <select value={sortBy} onChange={(event) => setSortBy(event.target.value as "recent" | "price" | "route") }>
      ...
      {favoritesQuery.isLoading ? <div className="skeleton-list"><div className="skeleton-card" /></div> : null}
      {favoritesQuery.isError ? <AuthMessage variant="error">We could not load your favorites. Please sign in again if needed.</AuthMessage> : null}
      {!favoritesQuery.isLoading && !favoritesQuery.isError && filteredFavorites.length === 0 ? (
